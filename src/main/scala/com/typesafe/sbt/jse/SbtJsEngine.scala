...
object JsEngineImport {
  ...
  object JsEngineKeys {
    ...
    val npmNodeModuleDirectory = SettingKey[File]("jse-npm-node-module-directory", "Where the npm node_modules folder lives.")
    ...
    val packageNodeModules = TaskKey[File]("jse-package-node-modules", "Package a plugin's node_modules folder if it exists.")
  }
...
object SbtJsEngine extends AutoPlugin {
  ...
  override def requires = SbtWeb && JvmPlugin
  ...
  private val jsEngineUnscopedSettings: Seq[Setting[_]] = Seq(
      ...
      val runUpdate = FileFunction.cached(cacheDirectory, FilesInfo.hash) {
        _ =>
          if (npmPackageJson.exists) {
            ...
            npmNodeModuleDirectory.value.***.get.toSet
          } else {
            IO.delete(npmNodeModuleDirectory.value)
            Set.empty
          }
      }
    ...
    nodeModuleDirectories += npmNodeModuleDirectory.value
  )

  private val defaultEngineType = EngineType.Trireme

  override def projectSettings: Seq[Setting[_]] = Seq(
    artifact in packageNodeModules := Artifact(name.value, "node_modules", "zip"),
    ...
    npmNodeModuleDirectory := baseDirectory.value / NodeModules,
    npmTimeout := 2.minutes,
    parallelism := java.lang.Runtime.getRuntime.availableProcessors() + 1,
    packageNodeModules := {
      val f = target.value / "xyz.zip"
      IO.zip((npmNodeModules in Assets).value.pair(relativeTo(npmNodeModuleDirectory.value.getParentFile)), f)
      f
    }

  ) ++ addArtifact(artifact in packageNodeModules, packageNodeModules).settings ++
